...
import {
  ...
  FileText, ShieldCheck, AlertCircle, Loader2, Sparkles,
} from 'lucide-react';
...
import {
  ...
  type ContributionAlgorithmVersion, type RecommendChoice, type ReadingSession,
} from '@/lib/types';
import {
...
type Phase = 'feedback' | 'recommend' | 'complete';
...
export function ReaderFeedback({ assignmentId, navigate }: ReaderFeedbackProps) {
  ...
  const [completionStatus, setCompletionStatus] = useState<FeedbackCompletion>('stopped_early');
    ...
    async function load() {
      ...
      if (a) {
        const [spRes, sessRes] = await Promise.all([
          supabase.from('screenplays').select('*').eq('id', a.screenplay_id).maybeSingle(),
        ...
        if (sessions.length > 0) {
          ...
          const totalActive = sessions.reduce((sum, s) => sum + (s.active_reading_seconds ?? 0), 0);
          setSessionData({
            scrollPosition: lastSession.scroll_position ?? 0,
            pagesRead: lastSession.last_page_reached,
            activeReadingSeconds: totalActive,
            lastPageReached: lastSession.last_page_reached,
          });
          setCompletionStatus(
            lastSession.last_page_reached >= (spRes.data as Screenplay)?.page_count
              ? 'completed'
              : 'stopped_early',
          );
        }
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-64 text-sm text-ink-400 dark:text-ink-500 animate-pulse">Loading feedback form...</div>;
  }
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-12">
        <Card className="p-8">
          ...
          <h2 className="text-xl font-bold text-ink-900 dark:text-white mb-2">Feedback already submitted</h2>
          <p className="text-sm text-ink-500 dark:text-ink-400 mb-6">
            You've already submitted feedback for this screenplay. You can view it in your Reading History.
          </p>
          <Button onClick={() => navigate('/reader/history')}>View Reading History</Button>
        </Card>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-8 space-y-6">
        ...
        <div className="space-y-3">
          <RecommendButton choice="recommend" current={recommendChoice} onSelect={setRecommendChoice}
            icon={<ThumbsUp className="w-5 h-5" />} label="Recommend" desc="I'd recommend this to others" color="forest" />
          <RecommendButton choice="consider" current={recommendChoice} onSelect={setRecommendChoice}
            icon={<Star className="w-5 h-5" />} label="Consider" desc="It has potential but needs work" color="accent" />
          <RecommendButton choice="pass" current={recommendChoice} onSelect={setRecommendChoice}
            icon={<ThumbsDown className="w-5 h-5" />} label="Pass" desc="I wouldn't recommend this" color="coral" />
        </div>
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-12">
        <Card className="p-8">
          ...
          {contributionAwarded > 0 && (
            <p className="text-sm font-medium text-accent-600 dark:text-accent-400 mb-6">
              +{contributionAwarded} contribution points awarded
            </p>
          )}
          <div className="flex flex-col gap-2">
            <Button onClick={() => navigate('/reader/contribution')}>View your contribution</Button>
            <Button variant="ghost" onClick={() => navigate('/reader')}>Back to assignments</Button>
          </div>
        </Card>
      </div>
    );
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <Card className="p-4">
        <div className="grid grid-cols-4 gap-4 text-center">
          ...
          <div>
            <div className="text-xs text-ink-400 dark:text-ink-500 uppercase tracking-wider mb-1">Sessions</div>
            <div className="text-lg font-bold text-ink-900 dark:text-white tabular-nums">{sessionCount}</div>
          </div>
          <div>
      ...
      <Card className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-2 text-xs text-ink-400 dark:text-ink-500">
          ...
          Completion: {completionStatus === 'completed' ? 'Read in full' : 'Stopped early'}
        </div>
